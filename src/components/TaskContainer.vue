<template>
  <div
    class="bg-gray-100  mb-2 flex bordered rounded-l-lg h-32 "
    :class="{ 'bg-red-500': taskData.prio, 'bg-green-600': !taskData.prio }"
  >
    <div class="flex items-center align-center mx-2 border-l-xs">
      <i :class="taskData.icon" class="text-gray-300"></i>
    </div>
    ...
    <div class="bg-white px-2 py-1 flex-grow relative">
      ...
      <p class="text-sm">
        Dodano: {{ taskData.addDate + " " + taskData.addTime }}
      </p>
      <!-- {{taskData}} -->
      <div
        class="bg-orange-500 text-center flex timer items-center pl-2"
        v-if="showTimer"
      >
        <i class="fas fa-stopwatch text-white mr-2"></i>
        <p v-if="!endOfTime" class="text-white">
    ...
    <div
      class="flex flex-col align-between justify-around pr-2 bg-gray-200 shadow-lg text-2xl px-2"
    >
      <button @click="deleteTask(taskData.id)">
        <i class="fas fa-times text-red-700"></i>
      </button>
      <button @click="() => (this.inEdit = !this.inEdit)">
    ...
    <EditTask
      ...
      :dateActive="date"
      v-if="inEdit"
      v-on:cancel="() => (this.inEdit = !this.inEdit)"
    />
  </div>
</template>
